...
public class DistressAlertService {
    ...
    public DistressAlertResponse getFarmerAlert(
        ...
        return alertRepository
                .findByIdAndFarmerId(alertId, farmer.getId())
                .map(this::toResponse)
                ...
                ? alertRepository.findAll()
                        ...
                        .sorted(
                                Comparator.comparing(
                                        DistressAlert::getCreatedAt).reversed())
                        .toList()
    ...
    public DistressAlertResponse acknowledge(
        ...
        try {
            alert.acknowledge(
                    ...
                    request.note().trim(),
                    LocalDateTime.now());
        } catch (AssignedOfficerConflictException exception) {
            throw new AssignedOfficerConflictException();
        } catch (InvalidAlertTransitionException exception) {
    ...
    private boolean hasActiveAlert(
        ...
        return alertRepository
                .findByFarmerIdAndCropIdAndConditionKeyAndStatus(
                        farmerId,
                        cropId,
                        conditionKey,
                        AlertStatus.OPEN)
    ...
    private DistressAlertResponse toResponse(
            DistressAlert alert) {
        ...
        return new DistressAlertResponse(
                ...
                alert.getStatus(),
                alert.getAssignedOfficer() == null
                        ? null
                        : alert.getAssignedOfficer().getId(),
    ...
    private String serializeFactors(
    ...
    private List<RiskFactor> deserializeFactors(
            String factors) {
    ...
    public static class AlertNotFoundException
    ...
    public static class InvalidAlertTransitionException
            extends RuntimeException {
    }

    public static class AlertSerializationException
            extends RuntimeException {
    }

    public static class AssignedOfficerNotFoundException
            extends RuntimeException {
    }

    public static class InvalidAssignedOfficerException
            extends RuntimeException {
    }

    public static class AssignedOfficerConflictException
            extends RuntimeException {
    }
}